...
| 규칙 | 이유 |
| --- | --- |
| `presentation`은 `data`를 import하지 않는다. 화면은 도메인 모델만 본다. | 서버 응답 형태가 바뀌어도 화면 코드가 안 바뀐다. |
...
| 모델 | **freezed 3** + `json_serializable` | 수동 `fromJson`: 필드 추가 때마다 실수 여지. |
| 로컬 저장 | `flutter_secure_storage` (토큰), `shared_preferences` (설정) | |
| 알림 | `firebase_core`, `firebase_messaging`, `flutter_local_notifications` | 포그라운드 표시는 local_notifications가 담당. |
| 카메라·권한 (후속) | `camera`, `permission_handler`, `image` 또는 `flutter_image_compress` | 이번 작업에서는 설치하지 않고 문서에만 기록. |
| lint | `very_good_analysis` + `riverpod_lint` + `custom_lint` | `flutter_lints`는 규칙이 느슨함. |
...
- Repository 구현체 교체(실제 ↔ 가짜)는 Provider override로 한다. `bootstrap.dart`에서 flavor 또는 dart-define 값에 따라 override 목록을 결정한다.
...
- Android: `productFlavors`로 `dev`/`prod`를 선언하고 `applicationIdSuffix ".dev"`를 붙여 두 flavor가 한 기기에 같이 설치되게 한다.
- iOS: Xcode scheme과 configuration을 `dev`/`prod`로 나누고 Bundle ID suffix를 동일하게 둔다.
...
- 설정 파일 위치
  ...
  - iOS: `ios/Runner/Firebase/dev/GoogleService-Info.plist`, `.../prod/...` 를 두고 Build Phase 스크립트가 configuration에 맞는 파일을 복사
- 설정 파일은 `.gitignore`에 넣고, 자리 표시용 `README.md`를 각 폴더에 둔다.
- `core/notification/`은 설정 파일이 없어도 앱이 뜨도록 초기화 실패를 무시하는 방어 코드를 둔다.
...
- `custom_lint` + `riverpod_lint`로 Riverpod 오용을 잡는다.
- 계층 규칙(2장)은 import 제한 lint로 강제한다. 구현 시점에 `custom_lint` 기반 도구 중 유지보수되는 것을 골라 쓰고, 마땅한 것이 없으면 `test/architecture_test.dart`에서 import 문을 정규식으로 검사하는 테스트로 대체한다. 어느 쪽이든 위반 시 CI가 실패해야 한다.
...
| 계층 규칙 | 필수 | lint 또는 architecture test |
...
- 단계: checkout → Flutter 설치 (`.fvmrc` 버전 사용) → `flutter pub get` → `build_runner build` → 생성 파일 diff 검사 → `flutter analyze` → `custom_lint` → `flutter test`.
...
- Flutter는 작업 시점의 최신 stable을 `.fvmrc`에 고정한다.
